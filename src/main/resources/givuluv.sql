...
select systemname from file where connectionid = 'farmart' and type = 'P' and boardthumbnail='N';
select * from product;
CREATE TABLE `seller` (
...
select * from product;
select * from file;
delete from seller where sellerid='banana';
CREATE TABLE `org` (
...
WHERE s.sellerid = 'seller' and p.p_type='M' limit 0,10;

SELECT p.*
FROM product p
JOIN s_board sb ON p.connectid = sb.s_boardnum
JOIN store s ON sb.s_num = s.s_num
WHERE s.sellerid = 'seller' and p.p_type='M' and p.productname like '%봄%';
...
select * from file;
select * from product where productname like '%봄%';
select * from product;
...
select b.*, p.* from d_board b join d_payment p on b.d_boardnum = p.d_boardnum where p.userid='apple';
select * from d_board;
select * from store;
select * from product;
select * from file;
...
select * from f_payment;
select b.*, f.* from f_board b join f_payment f on b.f_boardnum = f.f_boardnum where f.userid='apple';
...
select * from s_info;
insert into s_info values(1,'안녕',1);
create table `o_info`(
